...
const AppContainer = ({className}: Props) => {
	...
	const onVolumeChange = ({target}: Props): void => {
		if(target) {
			audioSummer.volume = Number(target.value)
			audioRain.volume = Number(target.value)
			audioWinter.volume = Number(target.value)
		}
	}
